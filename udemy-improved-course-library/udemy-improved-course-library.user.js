    ...
    [...courseContainers].forEach((courseContainer) => {
      ...
      fetch(fetchUrl)
        ...
        .then(json => {
          ...
          const updateDateShort = updateDate.replace(/(\d{4})-(\d{2})-(\d{2})/, '$2\/$1');
          const updateDateLong = new Date(updateDate).toLocaleDateString(lang, { year: 'numeric', month: 'long', day: 'numeric' });
          ...
          if (localeCode.slice(0, 2) !== 'en') {
            ...
            //console.log("Language is " + locale + ". Not showing Badge.")
          }
        ...
        .catch(error => {
          ...
          console.warn('Could not fetch data for "%s".', detailsName.textContent, error);
          //console.warn(error);
        });
    });
  }
